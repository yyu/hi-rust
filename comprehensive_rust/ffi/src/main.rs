...
impl DirectoryIterator {
    fn new(path: &str) -> Result<DirectoryIterator, String> {
        ...
        let path_bytes: Vec<i8> = path.bytes().map(|x| x as i8).collect();
        path_bytes.push(0);
        unsafe {
            let p: *const c_char = &raw const path_bytes[0];
            let dir = ffi::opendir(p);
            if dir == ptr::null() {
                return Err(String::new("opendir failed.")),
            } else {
